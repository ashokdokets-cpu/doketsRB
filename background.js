...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'saveJobToDokets') {
        const jobData = request.jobData || {};
        const title = encodeURIComponent(jobData.title || 'Saved Job');
        const company = encodeURIComponent(jobData.company || '');
        const url = 'https://www.doketsrb.com/#job-tracker?title=' + title + '&company=' + company;
