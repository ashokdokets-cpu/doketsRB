...
// ============================================
// 1. ADD ALL VIEWS (before Views['forgot-password'])
// ============================================
var allViews = `
...
async function loadResumeList(){if(!currentUser||!sbClient)return;var c=document.getElementById('resume-list');if(!c)return;try{var r=await sbClient.from('resumes').select('*').eq('user_id',currentUser.id).order('updated_at',{ascending:false});if(!r.data||r.data.length===0){c.innerHTML='<div class="col-span-full text-center py-12"><p class="text-gray-500 text-lg">No saved resumes yet.</p><button onclick="navigate(\'builder\')" class="mt-3 px-5 py-2.5 bg-brand-600 text-white rounded-lg font-bold">Create Your First Resume</button></div>';return}c.innerHTML=r.data.map(function(d){var s=d.composite_score||d.ats_score||0;var cl=s>=70?'text-green-600':s>=40?'text-amber-600':'text-red-600';return'<div class="bg-white rounded-xl p-5 border shadow-sm hover:shadow-md transition cursor-pointer" onclick="loadResumeById(\''+d.id+'\')"><h3 class="font-bold">'+(d.title||'Untitled')+'</h3><div class="flex items-center gap-2 mt-2"><span class="'+cl+' font-extrabold text-xl">'+s+'%</span><span class="text-xs text-gray-500">ATS</span></div><p class="text-xs text-gray-400 mt-3">Updated: '+new Date(d.updated_at).toLocaleDateString()+'</p></div>'}).join('')}catch(e){}}
...
html = html.replace("Views['forgot-password']", allViews + "\nViews['forgot-password']");

// ============================================
// 2. ADD LOGOUT + PLAN BADGE + NAV ITEMS
// ============================================
var navAdd = `
                    ...
                    <button onclick="if(currentUser){logOut()}else{navigate('login')}" class="px-3 py-2 text-sm font-medium rounded-lg text-red-600 hover:bg-red-50 transition">Logout</button>
`;
...
html = html.replace("function showError(m){", planBadge + "function showError(m){");

// Call updatePlanBadge on login
html = html.replace("showSuccess('Welcome back!');checkEmailVerification();navigate('dashboard');",
    "showSuccess('Welcome back!');checkEmailVerification();updatePlanBadge();navigate('dashboard');");

// ============================================
// 3. ADD DUPLICATE/RENAME/UNDO FUNCTIONS
// ============================================
var builderTools = `
var versionHistory=[];
function duplicateResume(){if(!enforceResumeLimit())return;var rd=JSON.parse(JSON.stringify(App.resumeData));rd.personal.fullName=(rd.personal.fullName||'Resume')+' (Copy)';updateState({resumeData:rd});showSuccess('Resume duplicated!')}
...
html = html.replace("function navigate(view){", builderTools + "\nfunction navigate(view){");

// Track changes for undo
html = html.replace("function updateState(changes) {\n        Object.assign(App, changes);",
    "function updateState(changes) {\n        versionHistory.push(JSON.parse(JSON.stringify(App.resumeData)));if(versionHistory.length>20)versionHistory.shift();\n        Object.assign(App, changes);");

// Add buttons to builder header
var builderBtns = `
...
<button onclick="renameResume()" class="px-3 py-2 bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200 transition font-semibold text-xs">Rename</button>
...
html = html.replace("← Dashboard</button><button onclick=\"exportPDF()\"", "← Dashboard</button>" + builderBtns + "<button onclick=\"exportPDF()\"");
...
console.log('ALL FEATURES ADDED: My Resumes, Profile, Subscription, Invoices, Logout, Plan Badge, Duplicate, Rename, Undo');